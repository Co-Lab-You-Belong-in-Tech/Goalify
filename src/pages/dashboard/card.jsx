...
import {
    progressI, dotsI, playI, eyeI, unprogressI, completeI, editI, deleteI, uneyeI, pauseI,
} from '../../assets/icons';
    ...
    return (<div className="w-80 p-2 border border-slate-400 w rounded">
        ...
        <div className="bg-gray-100 pb-1">
            <div className="rounded flex justify-between items-center py-2 px-1 mt-4">
              ...
              {getCurrentMilestone}/{goal.milestones.length} Milestones
              Reached
            </span>
                </div>
                {view ? (<span>
        ...
        <div className="bg-blue-50 py-5 rounded px-2 my-5 flex gap-3">
            {complete ? (<img className="w-5" src={completeI} alt="completeI"/>) : (<img
            ...
            <p className="text-sm">
                ...
                {getCurrentMilestone === goal.milestones.length ? 'CONGRATS You Finished Your Goal' : `Milestone   ${getCurrentMilestone + 1}: ${goal?.milestones[getCurrentMilestone]?.content?.substring(0, 24)}...`}
            </p>
        </div>
